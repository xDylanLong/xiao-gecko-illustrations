...
{只出现一个默认小睫毛或一个用户指定角色；角色主动指向、托住、拉开、揭示或回应标题。标题必须居中并作为视觉中心，直接取模块入口口播要呈现的问题或模块内容，保留原话，不凭空增加结论。}
